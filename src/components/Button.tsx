...
export function Button({
  ...
  return (
    <button
      type="button"
      className={clsx(
        "button-cutout group inline-flex items-center font-bold transition-all duration-300 hover:bg-bottom",
        // Sizes
        size === "sm" && "gap-1 py-2  text-base",
        size === "md" && "gap-1.5 py-2  text-lg",
        size === "lg" && "gap-1.5 py-2 text-lg",
        // Colors
        color === "orange" &&
          "bg-gradient-to-b from-brand-orange to-brand-lime text-black",
        color === "purple" &&
          " bg-brand-purple hover:bg-brand-lime text-white hover:text-black",
        color === "lime" &&
          "bg-gradient-to-b from-brand-lime to-brand-orange text-black",
        className
      )}
      {...props}
    >
      {icon && (
        <>
          ...
          <div className="w-px h-full bg-black/25 mx-2" />
        </>
      )}
      {children}
    </button>
